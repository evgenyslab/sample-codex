...
def extract_audio_metadata(filepath: Path) -> Dict:
    ...
    try:
        ...
        # Extract common tags
        if audio.tags:
            metadata['title'] = str(audio.tags.get('title', [''])[0]) if hasattr(audio.tags.get('title', ['']), '__iter__') else str(audio.tags.get('title', ''))
            metadata['artist'] = str(audio.tags.get('artist', [''])[0]) if hasattr(audio.tags.get('artist', ['']), '__iter__') else str(audio.tags.get('artist', ''))
            metadata['album'] = str(audio.tags.get('album', [''])[0]) if hasattr(audio.tags.get('album', ['']), '__iter__') else str(audio.tags.get('album', ''))
...
def scan_for_files(folder_paths: List[str], progress_callback: Optional[Callable] = None) -> List[Path]:
    ...
    with db.get_connection() as conn:
        for folder_path in folder_paths:
            conn.execute(
                "UPDATE folders SET status = 'processing' WHERE path = ?",
